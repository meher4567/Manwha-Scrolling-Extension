...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const waitImagesToggle = document.getElementById("wait-images-toggle");
  ...
  speedSlider.value = String(settings.scrollSpeed ?? 50);
  speedValue.textContent = speedSlider.value;
  autoAdvanceToggle.checked = settings.autoAdvanceChapter ?? true;
  waitImagesToggle.checked = settings.waitForImages ?? false;
  emergencyStopToggle.checked = settings.emergencyStopOnShake ?? true;
  ...
  speedSlider.addEventListener("change", async (e) => {
    const scrollSpeed = parseInt(e.target.value);
  ...
  autoAdvanceToggle.addEventListener("change", async (e) => {
    const autoAdvanceChapter = e.target.checked;
  ...
  waitImagesToggle.addEventListener("change", async (e) => {
    const waitForImages = e.target.checked;
  ...
  emergencyStopToggle.addEventListener("change", async (e) => {
    const emergencyStopOnShake = e.target.checked;
  ...
  const requestSpeedUpdate = async () => {
    ...
    if (tab.id) {
      try {
        const response = await browser.tabs.sendMessage(tab.id, { action: "getSpeedState" });
        if (response && response.baseSpeed !== void 0) {
  ...
  const updateSpeedDisplay = (baseSpeed, currentSpeed) => {
  ...
  const loadStats = async () => {
    try {
      ...
      const stats = result.readingStats || { totalTimeMs: 0, totalDistance: 0, chaptersCompleted: 0, sessionsCount: 0 };
      const hours = Math.floor(stats.totalTimeMs / 36e5);
      const minutes = Math.floor(stats.totalTimeMs % 36e5 / 6e4);
